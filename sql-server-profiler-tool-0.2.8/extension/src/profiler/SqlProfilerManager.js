...
class SqlProfilerManager {
    ...
    async startProfiling() {
        ...
        try {
            Logger_1.Logger.info('Starting profiling...');
            // Try to connect using selected mssql profile first
            const selectedProfile = this.getSelectedConnectionName();
            Logger_1.Logger.info('Selected profile for profiling', { profile: selectedProfile });
            if (selectedProfile) {
            ...
            this.startPolling();
            Logger_1.Logger.info('Profiling started successfully!');
        }
        catch (error) {
            this.isProfilering = false;
    ...
    async isAzureSqlDatabase() {
        ...
        try {
            ...
            if (version.toLowerCase().includes('azure')) {
                Logger_1.Logger.info('Detected Azure SQL Database from version string');
                return true;
            }
            ...
            try {
                ...
                Logger_1.Logger.info('Detected SQL Server (on-premise/managed instance) - server views accessible');
                return false;
            }
            catch (serverViewError) {
                Logger_1.Logger.info('Server views not accessible - likely Azure SQL Database');
                return true;
            }
        }
        catch (error) {
            Logger_1.Logger.error('Error detecting database type:', error);
            // Default to Azure SQL Database for safety (most restrictive)
            Logger_1.Logger.info('Defaulting to Azure SQL Database due to detection error');
            return true;
        }
    }
    async diagnoseSystemViews() {
        if (!this.pool) {
            return;
        }
        Logger_1.Logger.info('=== DIAGNOSING SYSTEM VIEWS AVAILABILITY ===');
        const viewsToTest = [
        ...
        for (const view of viewsToTest) {
            try {
                ...
                Logger_1.Logger.info(`✓ View accessible: ${view}`);
            }
            catch (error) {
                const errorMsg = error instanceof Error ? error.message : String(error);
                Logger_1.Logger.info(`✗ View NOT accessible: ${view} - ${errorMsg}`);
            }
        }
        Logger_1.Logger.info('=== END SYSTEM VIEWS DIAGNOSIS ===');
    }
    async createXESession() {
        if (!this.pool) {
    ...
    async collectResults() {
        ...
        try {
            ...
            const request = this.pool.request();
            request.timeout = 60000; // 60 seconds timeout for XE queries
            let result;
            try {
                ...
                fallbackRequest.timeout = 60000; // 60 seconds timeout for fallback XE query
            ...
            const newEvents = result.recordset.map((record, index) => {
                const event = {
            ...
            const filteredNewEvents = newEvents.filter(e => !existingKeys.has(`${e.timestamp}_${e.statement}`));
    ...
    getSelectedConnectionName() {
        const config = vscode.workspace.getConfiguration('sqlProfiler');
        return config.get('selectedConnection') || '';
    }
    /**
     * Sets the selected connection
     */
    async setSelectedConnection(profileName) {
        const config = vscode.workspace.getConfiguration('sqlProfiler');
     ...
     */
    correctAzureSqlServerFormat(serverName) {
     ...
     */
    async connectWithAutoRetry(sqlConfig, connection) {
        ...
        if (serverCorrection.wasChanged) {
            Logger_1.Logger.warn(`Auto-correcting Azure SQL server format: "${sqlConfig.server}" → "${serverCorrection.corrected}"`);
        ...
        try {
            ...
            if ((error.code === 'ENOTFOUND' || error.code === 'ESOCKET') &&
                ...
                Logger_1.Logger.warn(`Azure SQL server format error detected. Attempting auto-correction...`);
                try {
                    ...
                    await this.testBasicAccess();
                    Logger_1.Logger.info(`✅ Connection successful with corrected server format: "${serverCorrection.corrected}"`);
                    ...
                    Logger_1.Logger.error('Connection failed even with corrected Azure SQL server format', retryError);
            ...
            if ((error.code === 10054 || error.message?.includes('10054') || error.message?.includes('pre-login handshake'))
                ...
                Logger_1.Logger.warn(`SSL handshake failed (Error 10054). Attempting retry with trustServerCertificate: true...`);
                try {
                    // Close any existing pool
                    if (this.pool) {
                        await this.pool.close();
                    ...
                    await this.testBasicAccess();
                    Logger_1.Logger.info(`✅ Connection successful with trustServerCertificate: true. Consider adding this setting to your connection configuration.`);
                    // Show success message with configuration guidance
                    ...
                    Logger_1.Logger.error('Connection failed even with trustServerCertificate: true', retryError);
     ...
     */
    handleConnectionError(error, connection, sqlConfig) {
...
• TrustServerCertificate: ${sqlConfig.trustServerCertificate}`;
     ...
     */
    async connectUsingSelectedProfile() {
        const selectedProfile = this.getSelectedConnectionName();
     ...
     */
    async promptForPassword(profileName, username) {
        ...
        if (password) {
            ...
            const savePassword = await vscode.window.showQuickPick(['Yes, save password securely', 'No, ask every time'], {
...
Current connection works but Extended Events requires VIEW SERVER STATE permission.`;
     ...
     */
    async clearStoredPassword(profileName) {
        if (!this.context) {
        ...
        try {
            ...
            Logger_1.Logger.info(`Stored password cleared for connection profile: ${profileName}`);
        }
        catch (error) {
            console.error('Failed to clear stored password:', error);
            Logger_1.Logger.error(`Failed to clear stored password for ${profileName}`, error);
     ...
     */
    async getConnectionsWithStoredPasswords() {
        if (!this.context) {
            return [];
        }
        const connections = this.getMssqlConnections();
        const connectionsWithPasswords = [];
    ...
    async clearAllStoredPasswords() {
        ...
        Logger_1.Logger.info(`Cleared stored passwords for ${connectionsWithPasswords.length} connection profiles`);
    }
    /**
     * Connects using the selected mssql profile with connection pooling
     */
    async connectUsingPool() {
        const selectedProfile = this.getSelectedConnectionName();
        ...
        const serverCorrection = this.correctAzureSqlServerFormat(connection.server);
        if (serverCorrection.wasChanged) {
            Logger_1.Logger.info(`Server name corrected: ${connection.server} → ${serverCorrection.corrected}`);
        ...
        const config = vscode.workspace.getConfiguration('sqlProfiler');
        const poolConfig = config.get('connectionPool') || {};
        ...
        Logger_1.Logger.info('Pool configuration', {
            server: sqlConfig.server,
        ...
        try {
            ...
            this.currentPoolKey = this.poolManager.getActivePoolKeys().find(key => key.includes(selectedProfile.replace(/[^a-zA-Z0-9_]/g, '_')));
            Logger_1.Logger.info('Successfully connected using connection pool', {
                poolKey: this.currentPoolKey,
                connected: this.pool.connected
            });
        }
        catch (error) {
            Logger_1.Logger.error('Failed to get connection pool:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async connectUsingConnectionStringPool(sqlConfig) {
        const config = vscode.workspace.getConfiguration('sqlProfiler');
        const poolConfig = config.get('connectionPool') || {};
        const sqlConfigAny = sqlConfig;
        ...
        Logger_1.Logger.info('Pool configuration from connection string', {
        ...
        try {
            ...
            Logger_1.Logger.info('Successfully connected using connection string pool', {
                poolKey: this.currentPoolKey,
                connected: this.pool.connected
            });
        }
        catch (error) {
            Logger_1.Logger.error('Failed to get connection string pool:', error);
            throw error;
        }
    }
    /**
...
exports.SqlProfilerManager = SqlProfilerManager;
//# sourceMappingURL=SqlProfilerManager.js.map